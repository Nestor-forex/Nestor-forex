...
//
// Es la misma pantalla de la app hermana (Intradía) sin la línea del pivote de
// sesión: el barrido diario no lo calcula, y una línea vacía no aporta nada.
      ...
      <main style={{ flex: 1, padding: '16px 16px 48px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>
              {dec === 2 ? '1 pip = 0.01' : '1 pip = 0.0001'} · un cierre por día
            </div>
          </div>
          <span
            style={{
        ...
        <div className="card" style={{ padding: '10px 6px 6px' }}>
          <svg
            ...
            role="img"
            aria-label={`Últimos ${serie20.length} cierres diarios de ${setup.name}. Precio ahora ${f(precio)}, stop-loss ${f(sl)}, objetivo ${f(tp)}.`}
          >
            <defs>
            ...
            {/* contexto: soporte y resistencia. La etiqueta lleva un contorno del
                color del fondo para que la línea punteada no la atraviese, y así
                la línea puede seguir cruzando todo el ancho. */}
            {contexto.map((n) => (
              <line
                key={n.etiqueta}
                x1={X0}
          ...
          >
            <span>hace {serie20.length} días</span>
        ...
        <div className="card" style={{ borderLeft: `2px solid ${colorLado}`, display: 'flex', flexDirection: 'column', gap: 5 }}>
          ...
          <p style={{ margin: 0, fontSize: 13.5 }}>
            ...
            hacia la EMA20 (<span className="mono">{f(ema)}</span>).
          </p>
        </div>
