...
// Node (reporte diario y vigía).
//
//
// Devuelve la grilla { barras, rates, rangos } que espera marketCalc.js:
// solo los días que trajeron dato en las 7 divisas, para no dejar huecos si
// alguna no cotizó por feriado local.
...
export async function obtenerVelas(apiKey, { minBarras = 60 } = {}) {
  const r = await pedir(
    `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(SYMBOLS.join(','))}&interval=1day&outputsize=300&timezone=UTC&apikey=${apiKey}`
  )
